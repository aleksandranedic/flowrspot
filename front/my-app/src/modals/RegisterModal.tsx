...
const RegisterModal: React.FunctionComponent<ModalProps> = ({
  openModal,
  setOpenModal,
  setSuccessModal,
}) => {
  ...
  return (
    ...
    >
      ...
      <form
        ...
        className="modal-content flex flex-column"
      >
        ...
        <div
          className={`justify-center items-center gap-2 mt-2 ${
            showErrorRegister ? "flex" : "hidden"
          }`}
        >
          <MdOutlineError fill="#DF9186" size={18} />
          <p className="modal-error-message" ref={errorMessage}>
            {" "}
          </p>
        </div>
        <input
          type="submit"
          className="pink-button !rounded mt-10 mb-5 !text-base"
          value={"Create Account"}
        />
        <p className="beneath-modal" onClick={() => setOpenModal(false)}>
          {" "}
