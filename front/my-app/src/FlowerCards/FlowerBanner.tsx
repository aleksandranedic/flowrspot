...
const FlowerBanner: React.FunctionComponent<FlowerBannerProps> = ({data}) => {
    return ( <>
        <div className="relative w-full h-96 sm:h-fit">
                <img src="../images/flower-banner.png" alt="banner" className="banner h-96 object-contain hidden sm:flex w-full"/>
      
                <img src={data.flower.profile_picture} alt="profile" className="absolute w-full h-96 object-fill sm:top-32 sm:left-4 sm:rounded sm:w-80 brightness-50 sm:filter-none"/>
                ...
                <div className="flex gap-x-5 gap-y-3 justify-start flex-wrap w-fit absolute top-1/2 left-6 sm:top-40 sm:left-1/2 lg:left-1/4 lg:ml-10 lg:top-1/2">
                    ...
                    <SightingsButton favorite={data.flower.favorite} sightings={data.flower.sightings} />
                    <p className="font-ubuntu text-5xl !font-light text-white w-full"> {data.flower.name} </p>
                    <p className="font-ubuntu text-lg !font-light text-white w-full"> {data.flower.latin_name} </p>
                </div>
